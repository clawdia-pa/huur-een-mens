...
server.registerTool("search_humans", {
    title: "Search Humans",
    description: "Search for available humans on HuurEenMens. Optionally filter by skill, city, or hourly rate range. Returns a list of humans with their profiles.",
    inputSchema: {
        skill: z
            .string()
            .optional()
    ...
    if (min_rate !== undefined)
        params.set("minRate", String(min_rate));
    if (max_rate !== undefined)
        params.set("maxRate", String(max_rate));
    const query = params.toString();
    const data = await apiRequest(`/api/users${query ? `?${query}` : ""}`);
    const humans = data.humans || [];
    if (humans.length === 0) {
        return {
            content: [{ type: "text", text: "No humans found matching your criteria." }],
        };
    }
    const summary = humans.map((h) => [
        `Name: ${h.name}`,
        `ID: ${h.id}`,
        ...
        h.skills?.length ? `Skills: ${h.skills.map((s) => s.name).join(", ")}` : null,
    ]
        .filter(Boolean)
        .join("\n")).join("\n---\n");
    return {
...
server.registerTool("get_human", {
    ...
    description: "Get the full profile of a specific human by their ID. Returns detailed info including bio, skills, location, and hourly rate.",
    inputSchema: {
        human_id: z.string().describe("The unique ID of the human (e.g. 'user_17718...')"),
    },
}, async ({ human_id }) => {
    const data = await apiRequest("/api/users");
    const human = (data.humans || []).find((h) => h.id === human_id);
    if (!human) {
        return {
            content: [{ type: "text", text: `Human with ID '${human_id}' not found.` }],
        };
    }
    const profile = [
        `Name: ${human.name}`,
        ...
        human.skills?.length ? `Skills: ${human.skills.map((s) => s.name).join(", ")}` : null,
...
server.registerTool("list_skills", {
    title: "List Skills",
    description: "List all available skills and categories on HuurEenMens. Use the skill slugs when searching for humans.",
    inputSchema: {},
}, async () => {
...
server.registerTool("book_human", {
    title: "Book a Human",
    description: "Book a human for a physical task. Requires authentication (API token). You must provide the human's ID, a task description, location, date, and time.",
    inputSchema: {
...
// ─── Tool 5: get_bookings ───────────────────────────────────────────────────
server.registerTool("get_bookings", {
    title: "Get Bookings",
    description: "List all bookings for the authenticated user. Optionally filter by status (pending, confirmed, completed, cancelled).",
    inputSchema: {
        status: z
    ...
    if (bookings.length === 0) {
        return {
            content: [{ type: "text", text: status ? `No ${status} bookings found.` : "No bookings found." }],
        };
    }
    const summary = bookings.map((b) => [
        ...
        .join("\n")).join("\n---\n");
    return {
        content: [{ type: "text", text: `Found ${bookings.length} booking(s):\n\n${summary}` }],
    };
});
// ─── Tool 6: update_booking ─────────────────────────────────────────────────
server.registerTool("update_booking", {
    title: "Update Booking",
    description: "Update the status of a booking. Can be used to confirm, complete, or cancel a booking.",
    inputSchema: {
